package locator;

public class idObjects {

    //Home Page
    public static String LinkContact = "contact-link";
    public static String SearchBox = "searchbox";


    // Login Page
    public static String SubmitCreate = "SubmitCreate";
    public static String SubmitLogin = "SubmitLogin";


    public static String CreateEmail = "email_create";
    public static String SubmitBtnForNewUser = "SubmitCreate";

    // Registration Page
    public static String SelectGendrMr = "id_gender1";
    public static String NewPasswordCreation = "passwd";
    // ----Calendar
    public static String SelectDay = "days";
    public static String SelectMonth = "months";
    public static String SelectYear = "years";
    // -------
    public static String InputCompany = "company";
    public static String InputAddressFirst = "address1";
    public static String InputCity = "city";
    public static String InputMobilePhone = "phone_mobile";
    public static String InputPostCode = "postcode";
    public static String InputAlias = "alias";
    public static String InputState = "id_state";
    public static String SubmitNewAccount = "submitAccount";






//    public static String ContactLink = "contact-link";
//    public static String ContactLink = "contact-link";

}
